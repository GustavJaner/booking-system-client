...
function DateTimeslots({ setTimeslot, bookings, date, room }) {
  ...
  const createTimeslots = () => {
    ...
    var diffSec = moment(endTime, "HH:mm").diff(
    ...
    while (startTime !== endTime) {
      ...
      startTime = moment(
        moment(startTime, "HH:mm").add(roomDuration * 60, "seconds")
      ).format("HH:mm")
      slot.end = startTime
    ...
    bookings.forEach(booking => {
      for (let i = 0; i < timeSlotArr.length; i += 1) {
        if (
          timeSlotArr[i].start === booking.startTime &&
  ...
  const handleChange = event => {
    ...
    setTimeslot({
      start: event.target.value,
        ...
        moment(event.target.value, "HH:mm").add(room.duration * 60, "seconds")
      ).format("HH:mm")
    })
  }
